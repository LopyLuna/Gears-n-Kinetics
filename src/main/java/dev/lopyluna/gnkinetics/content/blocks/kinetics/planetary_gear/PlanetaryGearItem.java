...
public class PlanetaryGearItem extends BlockItem {
    public PlanetaryGearItem(Block block, Properties properties) {
    ...
    public @NotNull InteractionResult place(@NotNull BlockPlaceContext ctx) {
        ...
        if (clickedFace.getAxis() != ((PlanetaryGearBlock) getBlock()).getAxisForPlacement(ctx)) result = super.place(BlockPlaceContext.at(ctx, ctx.getClickedPos().relative(clickedFace), clickedFace));
    ...
    public void showBounds(BlockPlaceContext context) {
        ...
        var axis = ((PlanetaryGearBlock) getBlock()).getAxisForPlacement(context);
