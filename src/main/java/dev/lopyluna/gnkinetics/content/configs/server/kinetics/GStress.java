package dev.lopyluna.gnkinetics.content.configs.server.kinetics;
...
public class GStress extends ConfigBase {
    ...
    public static <B extends Block, P> NonNullUnaryOperator<BlockBuilder<B, P>> setImpact(double value) {
        return builder -> {
            ...
            DEFAULT_IMPACTS.put(Gears.loc(builder.getName()), value);
            return builder;
        };
    }
    ...
    public static <B extends Block, P> NonNullUnaryOperator<BlockBuilder<B, P>> setCapacity(double value) {
        return builder -> {
            assertFromCreateD2D(builder);
            DEFAULT_CAPACITIES.put(Gears.loc(builder.getName()), value);
            return builder;
        };
    }
    ...
    private static void assertFromCreateD2D(BlockBuilder<?, ?> builder) {
        if (!builder.getOwner().getModid().equals(MOD_ID)) {
            throw new IllegalStateException("Non-"+Gears.NAME+" blocks cannot be added to "+Gears.NAME+" config.");
        }
    }
